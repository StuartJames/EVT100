/* ansi_colours – true-colour ? ANSI terminal palette converter
   Copyright 2018 by Micha? Nazarewicz <mina86@mina86.com>
...
constexpr uint32_t colours[256] = {
// The 16 system colours as used by default by xterm.  Taken from XTerm-col.ad distributed with xterm source code. 
	0x000000, 0xcc0000, 0x4e9a06, 0xc4a000,	 // note these are personalised colours
	0x729fcf, 0x75507b, 0x06989a, 0xd3d7cf,
	0x555753, 0xef2929, 0x8ae234, 0xfce94f,
	0x32afff, 0xad7fa8, 0x34e2e2, 0xffffff,

// 6×6×6 cube.  One each axis, the six indices map to [0, 95, 135, 175, 215, 255] RGB component values. 
...
// Greyscale ramp.  This is calculated as (index - 232) * 10 + 8  repeated for each RGB component. 
...
static uint32_t CubeIndexRed(uint8_t r);
static uint32_t CubeIndexGreen(uint8_t g);
static uint32_t CubeIndexBlue(uint8_t b);
...
// Returns sRGB colour corresponding to the index in the 256-colour ANSI  palette. Corrected for windows
COLORREF RGBFromAnsi256(uint8_t Index)
{
int r = R(colours[Index]), g = G(colours[Index]), b = B(colours[Index]);
...
uint8_t Ansi256FromRGB(uint32_t RGB)
{
/* A lookup table for approximations of shades of grey.  Values chosen to get smallest possible.
...
static const uint8_t Ansi256FromGrey[256] = {
	...
	uint8_t GreyIndex = Ansi256FromGrey[Luminance(RGB)];
	uint32_t GreyDistance = Distance(RGB, RGBFromAnsi256(GreyIndex));
	uint32_t Cube = CubeIndexRed(R(RGB)) + CubeIndexGreen(G(RGB)) + CubeIndexBlue(B(RGB));
	return Distance(RGB, Cube) < GreyDistance ? Cube >> 24 : GreyIndex;
}
...
/* The next three functions approximate a pure colour by an entry in the 6×6×6
   ...
   motivated by ?E*?? being most variable in dark colours so I felt it’s more
...
static uint32_t CubeIndexRed(uint8_t v)
{
...
static uint32_t CubeIndexGreen(uint8_t v)
{
...
static uint32_t CubeIndexBlue(uint8_t v)
{
...
static uint8_t Luminance(uint32_t rgb)
{
	...
	/* Approximating sRGB gamma correction with a simple ?=2 improves the
