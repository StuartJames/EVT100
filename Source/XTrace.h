...
{
public:
	xtracing_output_debug_string(LPCTSTR lpszFile, int line) : m_file(lpszFile), m_line(line)
	{
	}
	...
	void operator() (LPCTSTR lpszFormat, ...)
	{
		...
		if(XTRACE_SHOW_THREAD_ID){
			if(_sntprintf(buf1, BUFFER_SIZE - 1, _T("%s(%d) : [%X] %s"), cp, m_line, GetCurrentThreadId(), lpszFormat) < 0)	buf1[BUFFER_SIZE - 1] = _T('\0');
		}
		else{
			if(_sntprintf(buf1, BUFFER_SIZE - 1, _T("%s(%d) : %s"),	cp, m_line, lpszFormat) < 0) buf1[BUFFER_SIZE - 1] = _T('\0');
		}
		// format the message as requested
		if(_vsntprintf(buf2, BUFFER_SIZE - 1, buf1, va) < 0) buf2[BUFFER_SIZE - 1] = _T('\0');
		va_end(va);
		if(XTRACE_FILE){
			...
			if(cp != NULL) *(cp + 1) = _T('\0');
			_tcscat(szPathName, _T("\\_trace.log"));
			HANDLE hFile = ::CreateFile(szPathName, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
			...
			OutputDebugString(buf2);			// write it out
		}
	}
...
{
public:
	xtracing_entry_output_debug_string(LPCTSTR lpszFile, int line) :	m_file(lpszFile),	m_line(line)
	{
	}
	...
	void operator() (LPCTSTR lpszFormat, ...)
	{
		...
		TCHAR buf1[BUFFER_SIZE];
		TCHAR* cp = (LPTSTR)m_file;		// add the __FILE__ and __LINE__ to the front
		if(!XTRACE_SHOW_FULLPATH){
			cp = (TCHAR*)_tcsrchr(m_file, _T('\\'));
			if(cp) cp++;
		}
		...
		if(_vsntprintf(buf2, BUFFER_SIZE - 1, buf1, va) < 0) buf2[BUFFER_SIZE - 1] = _T('\0');		// format the message as requested
		va_end(va);
		if(XTRACE_FILE){
			TCHAR szPathName[MAX_PATH * 2] = {0};
			::GetModuleFileName(NULL, szPathName, sizeof(szPathName) / sizeof(TCHAR) - 2);
